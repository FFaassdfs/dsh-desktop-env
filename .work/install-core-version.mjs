// Install script for dsh-client-ui-plugin-core-version.
...
//  3. Static verification: loader discovery conditions 1-4 (resolvable package,
//     dsh.client declaration, exports["./client"], host main exists) + patch
//     YAML parses and keeps the sibling plugin entries.
// Usage: node .work/install-core-version.mjs   (needs danger-full-access
...
const SRC = "D:/opencode/001/dsh-desktop/plugins/dsh-client-ui-plugin-core-version";
const DSH_HOME = join(process.env.USERPROFILE, ".dsh");
const PACKAGE_NAME = "dsh-client-ui-plugin-core-version";
const PATCH_PATH = join(DSH_HOME, "profiles", "web", "cordis.patch.yml");
const PATCH_ENTRY = `\n# dsh-client-ui-plugin-core-version: top-of-GUI core version badge.
...
if (patch.includes("plugin-core-version")) {
...
// condition 3: exports["./client"] exists and points at a real ModuleLoader bundle
...
// version route resolution works from the installed location
try {
  const probe = await import(pathToFileURL(mainPath).href);
  const version = probe.readCoreVersion ? probe.readCoreVersion() : "";
  console.log("3e. host readCoreVersion() -> " + (version === "" ? "(empty!)" : version));
} catch (error) {
  fail("host main import failed: " + error.message);
}
...
if (parsed === null) {
  console.log("3f. js-yaml not found — patch YAML check skipped");
} else {
  ...
  for (const expected of ["plugin-core-version", "plugin-explainer", "plugin-project-explorer"]) {
    if (!ids.includes(expected)) fail("patch entry missing after append: " + expected + " (have: " + ids.join(",") + ")");
  }
  console.log("3f. patch YAML OK (ids: " + ids.join(", ") + ")");
}

console.log("\nINSTALL OK — refresh the browser page to load the plugin.");
