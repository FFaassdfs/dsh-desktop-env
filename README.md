...
壳的状态面板里有「**核心版本**」一区，**按通道列出最新版本**，每个版本一个按钮：

```
核心版本  0.1.5-rc.2                    ↻
[rc]    0.1.7-rc.1      09-23 21:44  [更新] [跳过]
[alpha] 0.1.7-alpha.2   09-23 00:08  [更新] [跳过]
有可用更新（rc 0.1.7-rc.1）
```

- **壳自己绝不自动安装**：检查照跑（启动时 + 每 24h），但只会提示「有可用更新：…」，装不装由你点按钮决定。
- **按通道给最新版**：`rc` / `alpha`（**上游从来没有发过正式版**）。注意 dist-tags 会滞后——`latest` 可能比 `next` 旧（例：2026-09-23 `latest` 还是 `0.1.5-rc.3`，而 `next` 已是 `0.1.7-rc.1`），所以这里按**版本号比较**取每通道最新，而不是只看 `latest`。
- 「**跳过**」把该版本记进 `%APPDATA%\dsh-desktop\update.json`（`skippedCore`）：只有出现**比它更新**的版本时才会再提示；底部「恢复提醒」可撤销。
- 当前版本那一行是「**重装**」（修坏了可以用它重来）。
- 安装动作：脚本装走 `npm i -g @deepseek-ai/dsh@<版本>`；**便携包**则是把该版本下载进包内暂存目录，重启壳时换入（`--extract-runtime`/启动流程自带）。
- 版本号比较、通道归类、跳过记录的实现与测试见 `version.go` / `version_test.go`（`HANDOVER.md` §40）。
...
> 文档版本：v1.6（2026-09-24 更新）— 新增「核心版本（DSH 本体）的选择性更新」一节：壳状态面板新增「核心版本」区，**只提示不自动装**、按 semver 取各通道最新（`rc`/`alpha`，上游从无正式版）、逐版本「更新/重装」与「跳过」（跳过落盘可恢复），见 `HANDOVER.md` §40。
> v1.5（2026-09-23 更新）— 「只更新插件」补「用哪个 PowerShell」与 `--which-shell` 自检（`powershell` 永远是 5.1，PS7 只有 `pwsh.exe`）；安装器/更新器在 5.1 下也已验证可用（HANDOVER §34）。
