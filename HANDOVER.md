...
**§44 🔴 项目文件树「对不上当前项目目录」（2026-09-24）**——用户报 `project-explorer` 的树对应不上当前项目。实测定位：**会话列表快照没有 `current` 字段**（形状是 `{ids, byId, phase, projectionsBySession}`），而插件 **两处**（面板取根 + 拖拽插入）都读 `snap.current` ⇒ 恒 `undefined` ⇒ host 回退到自己进程 cwd，而快捷方式启动的 `dsh web` cwd 是**磁盘根 `C:\`** ⇒ 面板把整个 C 盘当项目目录。正确写法 = 官方 `dsh-client-ui-session` 的 `publishMain`：取 **`retainedBy.mainView > 0`** 的会话。同时修掉两个**测试基础设施**缺陷：①`.work/filetree-host.test.mjs` 一直**硬编码指向 `D:\opencode\001\dsh-desktop`（冻结旧工作区）**，本仓库 host 改动它根本看不见（已改相对路径）②host 回退到磁盘根无防护（现返回 **409 `no-project-root`**）。client 半区**刷新页面即生效**（按 mtime 算 rev）；host 守卫需**重启 dsh web**。测试：host A–O2 全过 + smoke 新增 8 例，并做**断层注入**证明断言有牙。见 §44。
...
1. ⏳ **用户目视验收**：刷新页面后面板标题应为 `dsh-desktop-env`（而非 `C:`），切换会话根目录跟随。
