...
## §44 🔴 项目文件树「对不上当前项目目录」：读了不存在的 `snap.current`（2026-09-24）

> **来源（用户报告）**：「project-explorer 这个插件不知道是什么原因，**无法对应上当前项目所在的目录了**，请查找原因」。
> 用户选定 **A 方案**（先恢复 shell 可用性，再用实测证据定位并修复）。

### 44.1 根因：会话列表快照**没有 `current` 字段**（一行代码恒取 `undefined`）

证据链（全部实测，非推断）：

| # | 检查 | 结果 |
|---|---|---|
| 1 | 探测**运行中**的 host 路由（带 `sessionCwd`） | `{"ok":true,"root":"D:\\dsh\\dsh-desktop-env","resolvedVia":"session"}` ⇒ **host 侧完全正常** |
| 2 | 同一路由**不带** `sessionCwd` | `{"ok":true,"root":"C:\\","resolvedVia":"fallback"}` ⇒ 回退到了 **`C:\`**（= `dsh web` 进程自身的 cwd） |
| 3 | 磁盘上的会话数据 | `storages/workspace.json` 里 workspace `path` = `D:\dsh\dsh-desktop-env`，**数据是对的** |
| 4 | 官方 `dsh-api-session-controller` 的 `this.list` | `createSnapshotStore({ ids, byId, phase, projectionsBySession })` ⇒ **没有 `current`** |
| 5 | 全文件搜 `current` | 所有命中都是**无关的局部变量**（`const current = event` 等），**无一处写入快照** |

插件的 client 半区原本写的是：

```js
const snap = sessions.list.getSnapshot();
const id = snap && snap.current;      // ← 永远是 undefined
if (id && snap.byId && snap.byId[id]) return snap.byId[id].cwd;
```

⇒ `currentSessionCwd()` 恒返回 `undefined` ⇒ 请求里 `sessionCwd` 缺失 ⇒ **host 回退到自己进程的 cwd**。而桌面壳/快捷方式启动的 `dsh web`，其 cwd 正是**磁盘根 `C:\`**，于是面板把整个 `C:\` 当成「项目目录」。

**这解释了全部现象**：树能显示、能展开、也不报错，只是**根的来源从"会话 cwd"退化成了"服务启动目录"**——所以看起来是"对不上当前项目"，而不是"坏了"。

### 44.2 正确判定方式：`retainedBy.mainView > 0`

官方 `dsh-client-ui-session` 的 `publishMain()` 给出了权威写法：

```js
const byId = this.sessions.list.getSnapshot().byId;
const currentId = this.current.value.key;
const nextId = currentId !== void 0 && (this.sessions.retainInfo(currentId).getSnapshot().retainedBy.mainView ?? 0) > 0
  ? currentId
  : Object.values(byId).find((c) => (c.retainedBy.mainView ?? 0) > 0)?.id;
```

即：**主视图正在保留（`retainedBy.mainView`）的那个会话**。`retainedBy` 由 session controller 投影进每个 `byId` 条目，**不需要任何额外服务**——只是原来读错了字段。

修复后的 `activeSessionId(sessions, preferredId)`（导出以便单测）：
1. 已跟随的会话**仍被主视图保留** → 继续用它（列表抖动时根不跳）；
2. 否则取**第一个** `retainedBy.mainView > 0` 的会话；
3. 都没有 → `undefined`。

### 44.3 同一 bug 的**第二处**：拖拽插入也读 `snap.current`

`insertPath()`（拖文件进对话框）里**同样**是 `const sessionId = snap && snap.current;` → 恒 `undefined` → 直接提示「请先打开一个会话」并**拒绝插入路径**。两份代码，同一个错误。

⇒ 已一并改用 `activeSessionId(sessions, shared.sessionId)`；新增 `shared.sessionId` 让面板与拖放处理器**用同一个会话**（保证"拖入路径的相对基准"与"用户看到的树"一致）。

> **教训**：同一个错误常量/字段被复制到两处时，修一处不够。这个仓库里已有先例（§3.6 的 `PROTECTED_IDS` 两份必须同步）；**修完要全文件搜一遍同名字段**。

### 44.4 顺带修掉的两个**测试基础设施**缺陷

1. 🔴 **`.work/filetree-host.test.mjs` 一直在测冻结的旧副本**：它写死了绝对路径
   `D:/opencode/001/dsh-desktop/plugins/.../lib/index.js`（**2026-09-14 冻结的旧工作区**）。
   实测两边文件**哈希不同**（frozen `C22444E6…` / repo `58073C38…`），且冻结副本**没有**任何新代码 ⇒ **本仓库对 host 半区的改动，这个套件根本看不见**（这也解释了为什么它此前"全绿"却没有发现过任何问题）。
   **已修**：改为相对本文件解析（`new URL("../plugins/...", import.meta.url)`）。
   > 这正是 §34.6-2「测试环境必须复刻目标条件」的镜像案例：**测试必须加载被测的那棵树**。
2. **host 回退到磁盘根没有任何防护**：`resolveRoot` 直接把 `realpath(process.cwd())` 当根返回。现在 `isFilesystemRoot()`（`C:` / `C:\` / `/`）命中时返回 **409 `no-project-root`** + 可读原因，前端显示该文案（不再渲染整个 C 盘）。**回退本身保留**（正常 cwd 仍然可用，用例 O1 守住）。

### 44.5 改动清单

| 文件 | 改动 |
|---|---|
| `plugins/.../src/bundle.template.js` | 新增 `activeSessionId(sessions, preferredId)`（官方 `mainView` 判定）+ `currentSessionCwd(sessions, preferredId)` 改用它；`insertPath` 同步改；新增 `shared.sessionId`；新增 `rootMessage` 状态把 host 的可读原因显示到面板；导出两个纯函数 |
| `plugins/.../lib/index.js` | 新增 `isFilesystemRoot()` 守卫 + 回退命中即 **409 `no-project-root`**；抽出 `errorCodeOf()` 让自定义 `code` 不被压成 `internal`（三处 handler 统一） |
| `plugins/.../lib/client.js` | 重建产物 **28,894 字节**（`node build.mjs`，**两次构建哈希一致**，可复现） |
| `.work/filetree-smoke.test.mjs` | 新增 **7 段共 8 例** `activeSessionId` 回归（含"快照确实没有 `current`"的前提断言、`mainView=0` 不选、非 mainView 保留不选、preferred 优先、失效后回退、空列表、服务缺失容错） |
| `.work/filetree-host.test.mjs` | **路径改为相对**（44.4-1）+ 新增 **O1/O2** 两例（普通 cwd 仍回退成功 / 磁盘根被 409 拒绝） |
| `plugins/.../README.md` | 根目录判定改为 `retainedBy.mainView` 并写明**快照没有 `current`**；补 409 语义；验收清单补"标题=会话 cwd" |

### 44.6 验证（全部实跑）

| 项 | 结果 |
|---|---|
| `node .work/filetree-host.test.mjs` | **A–O2 全过**（含新增 O1/O2；且从此测的是**本仓库**的 host） |
| `node .work/filetree-smoke.test.mjs` | **全过**，含 `activeSessionId OK (mainView derivation, 8 cases)` |
| **断层注入（证明断言有牙）** | 把产物里的 `mainView` 判定替换为常量 `0` 重跑 → **`activeSessionId did not pick the mainView session`，exit 1**；还原后全绿 |
| `node build.mjs` | 两次构建 `client.js` 哈希一致（`ADB2E2D9959A71CA`）⇒ 可复现 |
| 安装 | `node scripts/setup-plugins.mjs --plugins project-explorer` → `SETUP OK`；`--status` = **current**；安装副本与源码**逐字节同哈希**（client `ADB2E2D9…` / host `58073C38…`） |
| 实机路由（运行中实例） | 带 `sessionCwd` → `resolvedVia:"session"`，root = `D:\dsh\dsh-desktop-env` ✅ |
| **用户实视（2026-09-24）** | **只刷新页面** ⇒ 面板标题由 `C:` 变为 `dsh-desktop-env` ✅（用户确认；见 44.7-1） |

⚠️ **生效条件（分两半，务必分清）**：
- **client 半区**：宿主按 **mtime 内容 revision** 每次请求重读 `lib/client.js`（`artifactRevision` 用 `mtimeMs/ctimeMs`）⇒ **刷新页面（Ctrl+F5）即可生效**，无需重启壳。
- **host 半区（本次新增的 409 守卫）**：`index.js` 是**进程内加载**的 —— 实测运行中的 dsh web（PID 14372，12:40 启动）仍返回旧行为 `{"root":"C:\\"}`，而磁盘上的修复写于 13:54 ⇒ **必须完全重启 dsh web 才生效**。
- 🔴 **但注意**：**根因（读不到会话 cwd）的修复全在 client 半区**，所以**只刷新页面就已经修好"对不上当前目录"**；409 守卫只是"万一又拿不到会话时的兜底显示"。

### 44.7 遗留 / 待办

1. ✅ **用户目视验收已通过（2026-09-24）**：**只刷新页面（Ctrl+F5）** 后面板标题即从 `C:` 变为 `dsh-desktop-env`，用户确认「刷新后正确了」。
   - 这条同时**实测证实了 §44.6 的分半结论**：根因修复全在 **client 半区**，宿主按 mtime 重算 rev ⇒ **刷新页面即生效，无需重启壳**；host 半区的 409 守卫仍待重启才生效（非必需）。
   - 依据项目惯例（对照 §21.3 / §38.5）：**client 半区以用户实视为准**——命令行 `/plugins/*/client.js` 在未认证请求下 404，不能用来判断插件是否生效。
   - 📌 **仍未做（可选，非阻塞）**：重启 `dsh web` 让 409 `no-project-root` 守卫生效（仅为"万一又拿不到会话"时给出更清晰的报错；§44.1 的根因已由 client 修复消除）。
2. ⏳ **重启 dsh web 后**，409 守卫才会生效（可选，仅为更清晰的报错）。
3. 📌 **`setup-plugins.mjs` 的 `3b'` 警告是既有误报**（本次确认、未改）：它从 profile 解析 `dsh.client.inject` 的边，而 `profiles/node_modules/@deepseek-ai/<pkg>` 是**空占位目录**（真实模块在全局 dsh 安装树里）⇒ 一律报 `inject edge not resolvable`。**不是本次引入**（`package.json` 未改动），也不影响加载。若日后要消除，应改为从**全局 dsh 安装树**解析或按 dsh loader 的真实 baseUrl 解析。

### 44.8 📣 给其他会话：本次改动对「打包 / 发版 / 更新」的影响（已实测）

> 每个会话都可能重新打包或发版，**请先读这一节**，免得以为"重新打包会把别人的修复冲掉"或反过来漏掉修复。

**结论：不会互相冲掉。**「重新打包」在这个仓库里是两件不同的事，实测如下：

| 操作 | 是否碰 `plugins/` | 实测结果 |
|---|---|---|
| `wails build` / `update.ps1` 的**壳构建** | ❌ 不碰 | 构建输入只有 Go 源码 + `frontend/`（见 `wails.json`）；`plugins/` 不在其中 |
| `update.ps1` 第 2 步「刷新插件」 | ✅ 读 | 是 `setup-plugins.mjs` **从仓库源码拷到 `$DSH_HOME`**，不是重新构建 ⇒ 拷的就是仓库当前内容 |
| `scripts/pack-release.ps1`（本地打包） | ✅ 整目录拷 | 第 238–239 行 `Get-ChildItem $pluginsSrc -Directory \| Copy-Item -Recurse` ⇒ **仓库里是什么，包里就是什么** |
| CI（`release-desktop.yml`） | ✅ 从 git 取 | **从 tag/commit 取源码** ⇒ ✅ 已提交的修复会进包；⚠️ **未提交的改动不会** |

**本节的实测证据**（2026-09-24）：

```
# 跑 update.ps1 的插件步骤后，安装副本与仓库源码逐字节一致：
installer copy hash AFTER : ADB2E2D9959A71CA
src copy hash             : ADB2E2D9959A71CA
修复仍在（activeSessionId/mainView）: True
```

**🔴 三条纪律（照做即可互不干扰）**：

1. **`lib/client.js` 是入库的构建产物**。改了 `src/bundle.template.js` **必须重跑** `node plugins/<pkg>/build.mjs`，否则包里是**新模板 + 旧产物** ⇒ 修复静默失效。本次已重跑，并验证**两次构建哈希一致**（`ADB2E2D9959A71CA`，可复现）。
2. **要发版就先提交**。CI 从 commit 取源码，工作区里没提交的修复**不会**进发布包（本地 `pack-release.ps1` 会进，于是"本地包对、CI 包错"——这正是 §40.7 那类踩法的近亲）。
3. **别用 `git add -A`**（§32.1 的真实事故：把 5 个垃圾目录提交进了 `3f30a63`）。本次只 `git add` 了 7 个明确文件。

**本次提交涉及的文件**（若你之后要改同一个插件，请以这些为基线）：

```
plugins/dsh-client-ui-plugin-project-explorer/src/bundle.template.js   # activeSessionId/mainView 判定（根因）
plugins/dsh-client-ui-plugin-project-explorer/lib/client.js            # 重建产物 28,894 字节
plugins/dsh-client-ui-plugin-project-explorer/lib/index.js             # 磁盘根守卫 + errorCodeOf
plugins/dsh-client-ui-plugin-project-explorer/README.md                # 判定说明 + 409 语义 + 验收清单
.work/filetree-smoke.test.mjs                                          # +8 例回归
.work/filetree-host.test.mjs                                           # 路径改相对 + O1/O2
HANDOVER.md                                                            # 本节（§44）
```
