...
cd D:\dsh\dsh-desktop-env
# 推荐（PowerShell 7）：
pwsh -File deploy.ps1 -HarnessVersion 0.1.5-rc.2
# 或系统自带（Windows PowerShell 5.1）：
powershell -File deploy.ps1 -HarnessVersion 0.1.5-rc.2
```
...
```powershell
# 桌面壳方式：**唯一启动入口**（setup.ps1 / update.ps1 都部署到这里）
D:\dsh\app\current\dsh-desktop.exe
# 或纯 Web 方式
dsh web
```

> 构建产物本身在 `D:\dsh\dsh-desktop-env\build\bin\dsh-desktop.exe`，但那只是中间产物——**启动请用应用区那份**（脚本会自动部署；手动构建时可跑 `scripts\deploy-shell.ps1 -BuiltExe build\bin\dsh-desktop.exe`）。
...
- 只想更新依赖环境/插件：`pwsh -File deploy.ps1 -HarnessVersion 0.1.5-rc.2`
...
> 版本号两台机器保持一致（当前锁定 `0.1.5-rc.2`；升级时两台的 `-HarnessVersion` 一起改）。权威源见 `project-facts-v1.0.md` F10。
