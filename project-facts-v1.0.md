...
> **文档版本：v1.0**（2026-09-14 创建）
> 变更记录：v1.0 — 建立事实源索引，收口端口 / 核心版本 / 同步频率 / 路径 / 插件清单 等被多处复制的数值；勘误「官方同步每小时」（实为每天 08:00，见 F4）。
...
| F7 | **应用区（exe）** | 部署约定 | `D:\dsh\app\current\dsh-desktop.exe`；历史版本 `D:\dsh\app\versions\<日期>\` + `VERSION.txt` | `AGENTS.md`、`HANDOVER.md` §20 |
