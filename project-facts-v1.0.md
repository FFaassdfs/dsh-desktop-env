...
| F15 | **便携发行包（离线一键）** | `scripts/pack-release.ps1`（打包；`-RuntimeMode auto\|dsh-tree\|full-node-modules` + **运行时自检闸门**）+ `.github/workflows/release-desktop.yml`（打 `desktop-v*` tag 自动发 Release）+ `install-offline.ps1`（包内安装器，`-Plugins all\|none\|ask\|列表`） | 资产 `dsh-desktop-<ver>-dsh<dshver>-win-x64.zip` + `SHA256SUMS.txt`；含壳 exe + **便携 Node** + **离线 harness 树** + 4 插件。实测本地 zip 106.2 MB / 发布资产 104.9 MB（原始 ~312 MB）。目标机零前置依赖；⚠️ 壳是**单实例**（`SingleInstanceLock`）；⚠️ **`npm i -g` 是嵌套布局、`npm install --prefix` 是提升布局**——只拷 dsh 一棵树在前者完整、在后者是空壳（首发坏包事故与闸门见 `HANDOVER.md` §27.7） | `README.md`「便携发行包」、`HANDOVER.md` §27 |
